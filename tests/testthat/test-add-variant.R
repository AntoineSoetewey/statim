...
test_that("add_variant %<-% registers variant into registry", {
    add_variant(T_TEST, x_by, "test_simple") %<-% simple_variant
    on.exit(remove_variant(T_TEST, x_by, "test_simple"))
...
test_that("add_variant %<-% silently replaces existing user variant", {
    ...
    expect_no_error(add_variant(T_TEST, x_by, "test_simple") %<-% replacement)
...
test_that("add_variant errors on 'default' name", {
    expect_error(
        add_variant(T_TEST, x_by, "default") %<-% simple_variant,
        class = "rlang_error"
    )
})
...
test_that("add_variant errors when value is not a variant object", {
    expect_error(
        add_variant(T_TEST, x_by, "test_simple") %<-% function() {},
        class = "rlang_error"
    )
})
...
test_that("remove_variant removes a user-scoped variant", {
    add_variant(T_TEST, x_by, "test_simple") %<-% simple_variant
    remove_variant(T_TEST, x_by, "test_simple")
...
test_that("remove_variant errors on unregistered name", {
    expect_error(
        remove_variant(T_TEST, x_by, "nonexistent"),
        class = "rlang_error"
    )
})

test_that("remove_variant errors on 'default' name", {
    expect_error(remove_variant(T_TEST, x_by, "default"), class = "rlang_error")
})
...
test_that("remove_variant errors on package-scoped variant", {
    ...
    on.exit({
        variant_registry[[key]][["pkg_variant"]] = NULL
    })

    expect_error(
        remove_variant(T_TEST, x_by, "pkg_variant"),
        class = "rlang_error"
    )
})
...
test_that("T_TEST remains a function after add_variant %<-%", {
    add_variant(T_TEST, x_by, "test_simple") %<-% simple_variant
    on.exit(remove_variant(T_TEST, x_by, "test_simple"))

    expect_true(is.function(T_TEST))
    expect_equal(attr(T_TEST, "cls"), "ttest")
})
