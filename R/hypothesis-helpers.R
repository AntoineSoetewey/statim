...
#'
#' @param claim A `null_claim` object.
#' @param filter Optional. The name of a conditioning slot (e.g. `"given"`)
#'   that every `param_obj` term must supply. If a term's class declares
#'   that slot as an optional constructor argument (default `NULL`) but the
#'   slot is unset on the node, an error is raised. If `NULL` (the default),
#'   the slot to enforce is auto-detected per term by inspecting its class
