...
ttest_def_formula = test_define(
    ...
    vars = list(
        ...
        formula = function(p) p$formula
    ),
    run = function(self) {
        data = ic_pull(self, "data")
        formula = ic_pull(self, "formula")
        mu = ic_arg(self, ".mu")
        alt = ic_arg(self, ".alt")
        ci_level = ic_arg(self, ".ci")
    ...
    print = function(x, ...) {
        ...
        tidy_rows = lapply(seq_len(nrow(dat)), function(i) {
            ...
            # one-sample has `estimate` (mean of x); two-sample has `estimate`
            # (mean diff). Both are named `estimate` in broom output, consistent enough.
