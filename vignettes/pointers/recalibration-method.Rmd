...
Because matching is scoped to model type, the same variant name can mean different things (or simply do not exist) depending on which `<model_id>` you used in `define_model()`. A variant registered for `x_by()` pipelines is not automatically available to a `<formula>`-based pipeline of the same test, even if both eventually call `TTEST`.
...
If a hypothesis was attached with `state_null()`, recalibrating the method does not invalidate it, as long as the variant you switch to actually has a `claim_translator` registered for it. Each `stat_define()` can register a `claim_translator` that knows how to turn a stated claim into the arguments a specific variant's `fn` expects, since different estimation methods can require the same population-parameter claim expressed differently. `TTEST`'s `x_by()` implementation, for instance, registers a `claim_translator` for its `default` method and for `"contrast"`, but not for the resampling-based `"boot"` or `"permute"` variants, since those do not take a hypothesized value as an argument in the first place.
...
At `conclude()`, the translator registered for the `"contrast"` variant runs against the stated claim, and its output (in this case, `.mu`, `.op`, and `.w`) is merged into the argument list the same way the rest of `via()`'s arguments are. If a `claim_translator` is not registered for the variant you switched to, `conclude()` reports that explicitly rather than silently ignoring the claim:
...
-   A stated null hypothesis survives recalibration only if the new variant has a registered `claim_translator`, not every variant needs one.
