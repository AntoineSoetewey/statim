...
The pipeline is the same as for linear models, with `GLM` in place of `LINEAR_REG` and `update()` to supply the family:

```{r}
mod1 = mtcars |> define_model(am ~ wt) |> prepare_model(GLM) |> update(family = binomial())
mod2 = mtcars |> define_model(am ~ wt + hp) |> prepare_model(GLM) |> update(family = binomial())
